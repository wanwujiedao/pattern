...
###### Main是我的演示类，使用 FactoryProducer 来获取 AbstractFactory 对象。它将向 AbstractFactory 传递液体信息 Iliquid（Gasoline / Perfume），以便获取它所需对象的类型。同时它还向 AbstractFactory 传递用处信息 IUse（GasolineUse / PerfumeUse），以便获取它所需对象的类型。
